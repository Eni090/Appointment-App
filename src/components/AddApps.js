...
const AddApps = ({ onSendAppointment, lastId }) => {
	const clear = {
		ownerName: '',
		petName: '',
		aptDate: '',
	...
	let [ formData, setFormData ] = useState(clear);

	function formDataPublish() {
		const appointmemtInfo = {
			id: lastId + 1,
		...
		onSendAppointment(appointmemtInfo);
		setFormData(clear);
		setToggle(!toggle);
	}
	...
	return (
		<div>
			<button
				...
				className={`bg-blue-400 text-white px-2 py-3 w-full text-left rounded-t-md ${toggle
					? 'rounded-t-md'
					: 'rounded-md'}`}
			>
				<div>
